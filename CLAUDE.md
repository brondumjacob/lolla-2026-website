...
**In progress (on the `expansion` branch, not merged into `main`/production):** a migration to Supabase (Postgres + Auth) + Next.js + Vercel for accounts, saved schedules, and multi-festival support — see `2026-07-09_lolla-accounts-migration-plan.md` for the full plan and current phase status. Phases 0–4 are done: the Next.js app (`web/`) is live at `lolla-2026-website.vercel.app` (a Vercel preview domain, not the production domain fans use) with real Supabase data, and Google OAuth is wired end-to-end pending Google/Supabase dashboard provider setup. The repo-root static site is untouched by this work and keeps deploying to production via Cloudflare Pages exactly as before — no DNS/domain cutover has happened.
...
Driven by a repeat Google AdSense "low value content" rejection. All 5 phases of the plan (search + full artist descriptions + content depth) are complete; not yet committed or deployed — awaiting user go-ahead.
