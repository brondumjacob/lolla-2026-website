...
(function () {
  ...
  function getSaved() {
    try { return new Set(JSON.parse(localStorage.getItem(SKEY) || '[]')); }
  ...
  function renderMyLineupIfPresent() {
    ...
    root.innerHTML = picks.map(function (a) {
      return '<div class="mylineup-row">' +
        ...
        '<span class="ml-name">' + a.n + '</span>' +
        '<span class="ml-genre">' + a.g + '</span>' +
        '<div class="streaming-links">' + streamLinks(a) + '</div>' +
        '<button class="star-toggle saved" data-name="' + a.n + '" aria-label="Remove ' + a.n + ' from your lineup" title="Remove from My Lineup">\u2605</button>' +
        '</div>';
    }).join('');
  }
