...
- **Phase 6 (multi-schedule export):** Not started.
...
- **Git state as of this update:** `git log` shows commits through Phase 4 (`8f8ebdd`, `6bee9ef`, `b770acc`) plus the Worship description fix (`e652eff`). **Phase 5's changes are verified but sitting uncommitted in the working tree** (`web/app/favorites`, `web/app/my-lineup`, deleted `web/public/favorites.js`, several modified files, plus this plan doc) — confirmed directly via `git status`, not just taken on report.
